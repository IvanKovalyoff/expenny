export const calculateSubscriptionMetrics = (subscriptions) => {
        ...
        if (billingFrequency === "One-time") {
            ...
            while (next_billing_date < today) {
                if (billingFrequency === "Monthly") {
                    ...
                    break; // unknown frequency — exit to avoid infinite loop
                }
            }
    ...
    const average_monthly_spending = active_subscriptions.length > 0
        ? total_monthly_cost / active_subscriptions.length
        : 0;

    const top_spending_category = Object.entries(category_spending)
        .reduce((top, current) => current[1] > top[1] ? current : top, ["", 0])[0] || "None";
